  ...
  return (
    <section id="experience" className="section bg-gray-50 dark:bg-dark-800">
      <div className="container">
        ...
        >
          {/* Timeline line */}
          <div className="absolute left-0 md:left-1/2 top-0 bottom-0 w-1 bg-gradient-to-b from-primary-500 to-secondary-500 transform md:translate-x-[-50%]"></div>
          ...
          {experiences.map((exp, index) => (
            ...
            >
              ...
              <div className={`md:w-1/2 ${index % 2 === 0 ? 'md:pr-12 md:text-right' : 'md:pl-12'}`}>
                <h3 className="text-2xl font-bold mb-1 text-gray-800 dark:text-gray-100">{exp.title}</h3>
                <h4 className="text-lg font-medium mb-4 text-primary-600 dark:text-primary-400">{exp.company}</h4>
                ...
                <ul className={`space-y-2 text-gray-600 dark:text-gray-300 ${index % 2 === 0 ? 'md:text-right' : ''}`}>
                  {exp.description.map((item, i) => (
                    <li key={i} className="flex items-start md:items-center gap-2">
                      {index % 2 === 0 && <span className="hidden md:inline flex-grow"></span>}
                      <span className="w-1 h-1 mt-2 rounded-full bg-primary-500 flex-shrink-0"></span>
                      <span>{item}</span>
                      {index % 2 !== 0 && <span className="hidden md:inline flex-grow"></span>}
                    </li>
                  ))}
              ...
              <div className="absolute left-0 md:left-1/2 transform md:translate-x-[-50%] mt-2">
